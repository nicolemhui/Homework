...
class Play
  ...
  def self.find_by_playwright(name)
    ...
    # playwright = PlayDBConnection.instance.execute("SELECT * FROM plays WHERE name = #{name}")
  end

  def initialize(options)       #includes options hash
    @id = options['id']
    @title = options['title']
    @year = options['year']
...
class Playwright
  ...
  def create
    ...
    #id will automatically be created so we don't need to insert that into query
